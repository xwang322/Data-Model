...
with open('predictions_missing_after_dimension_extraction.txt', 'r') as f:
    ...
    for i in lines:
        ...
        if color[r+1] == 'null':
            for each in freq_color:
                if each in matrix[r+1][2]:
                    color[r+1] = each
                elif each in matrix[r+1][6]:
                    color[r+1] = each
...
for r in range(i+1):
    ...
    if FVmatrix[r][0] == 1 or FVmatrix[r][0] == 0:
        color_total += 1
    if (FVmatrix[r][0] == 1 and FVmatrix[r][1] == 1) or (FVmatrix[r][0] == 0 and FVmatrix[r][1] == 0):
        color_diff += 1 
	'''	
#print(FVmatrix)
#print(color_total, color_diff)
with open('predictions_missing_after_dimension_extraction.txt', 'r') as f:
    r = 0
    a = open('predictions_missing_after_color_extraction.txt','w')
    b = open('predictions_obtained_after_color_extraction.txt','w')
    lines = f.readlines()
    for i in lines:
        if FVmatrix[r][0] != 999:
            if FVmatrix[r][0] == 1:
...
f.close()
i = 0
with open('predictions_missing_after_color_extraction.txt', 'r') as f:
    for i, l in enumerate(f):
        pass
f.close()
print(i + 1)
original = []
with open('predictions_after_dimension.txt', 'r') as f:
    lines_target = f.readlines()
    for each in lines_target:
        original.append(each)
    with open('predictions_obtained_after_color_extraction.txt', 'r') as g:
    ...
    #print(original)	
f.close()	
with open('predictions_after_color.txt', 'w') as f:
