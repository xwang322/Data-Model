...
with open('predictions_missing_after_ID_extraction.txt', 'r') as f:
    r = 0
    a = open('predictions_missing_after_dimension_extraction.txt','w')
    b = open('predictions_obtained_after_dimension_extraction.txt','w')
    lines = f.readlines()
    for i in lines:
        if FVmatrix[r][0] != 999:
            if FVmatrix[r][0] == 1:
...
print(i + 1)
original = []
with open('predictions_after_ID.txt', 'r') as f:
    lines_target = f.readlines()
    for each in lines_target:
        original.append(each)
    with open('predictions_obtained_after_dimension_extraction.txt', 'r') as g:
    ...
    #print(original)	
f.close()	
with open('predictions_after_dimension.txt', 'w') as f:
