...
for r in range(i+1):
    ...
    if(matrix[2*r][1] == '' and matrix[2*r+1][1] == ''):
        ...
        FVmatrix[r][0] = simfunctions.jaro(matrix[2*r][1], matrix[2*r+1][1])		

    # product name: jaro distance
    if(matrix[2*r][2] == '' and matrix[2*r+1][2] == ''):
        FVmatrix[r][1] = 999
    else:
        FVmatrix[r][1] = simfunctions.jaro(matrix[2*r][2], matrix[2*r+1][2])

    # product segment: jaro distance
    if(matrix[2*r][3] == '' and matrix[2*r+1][3] == ''):
        FVmatrix[r][2] = 999
    else:
        FVmatrix[r][2] = simfunctions.jaro(matrix[2*r][3], matrix[2*r+1][3])

    # product brand: jaro distance
    if(matrix[2*r][4] == '' and matrix[2*r+1][4] == ''):
        FVmatrix[r][3] = 999
    else:
        FVmatrix[r][3] = simfunctions.jaro(matrix[2*r][4], matrix[2*r+1][4])

    # product category: jaro distance
    if(matrix[2*r][5] == '' and matrix[2*r+1][5] == ''):
        FVmatrix[r][4] = 999
    else:
        FVmatrix[r][4] = simfunctions.jaro(matrix[2*r][5], matrix[2*r+1][5])
...
x = open('5000Sampling8_string_processed_feature_vector.txt','w')
