...
feature_attr = ['id','product type','product name','product segment','brand','category','product short description']
...
with open('5000Sampling.txt','r') as f:
    ...
    for i in lines:
        ...
        matrix[r][6] = dict_tmp1.get('product short description')
		# for product 2
        ...
        matrix[r+1][6] = dict_tmp1.get('product short description')
        result[m] = items[5]
        r += 2
        m += 1		
#print(len(matrix))
...
for r in range(i+1):
    ...
    if(matrix[2*r][5] == '' and matrix[2*r+1][5] == ''):
        ...
        FVmatrix[r][17] = simfunctions.needleman_wunsch(matrix[2*r][5], matrix[2*r+1][5])
    # category: jaro_winkler distance
    if(matrix[2*r][5] == '' and matrix[2*r+1][5] == ''):
        FVmatrix[r][18] = 999
    else:
        FVmatrix[r][18] = simfunctions.jaro_winkler(matrix[2*r][5], matrix[2*r+1][5])  		
    ...
    FVmatrix[r][19] = simfunctions.soft_tfidf(set(product_category1), set(product_category2))
    # category: jaccard distance
    FVmatrix[r][20] = simfunctions.jaccard(set(product_category1), set(product_category2))

    # short description: jaro_winkler distance
    if(matrix[2*r][6] == '' and matrix[2*r+1][6] == ''):
        FVmatrix[r][21] = 999
    else:
        FVmatrix[r][21] = simfunctions.jaro_winkler(matrix[2*r][6], matrix[2*r+1][6])  		
    product_short1 = string_process2.string_process2(matrix[2*r][6])
    product_short2 = string_process2.string_process2(matrix[2*r+1][6])
    # short description: cosine distance
    FVmatrix[r][22] = simfunctions.cosine(set(product_short1), set(product_short2))
    # short description: overlap_coefficient distance
    FVmatrix[r][23] = simfunctions.overlap_coefficient(set(product_short1), set(product_short2))
...
x = open('5000Sampling22_string_processed_feature_vector.txt','w')
