...
r = 0
for r in range(i+1):
    FVmatrix[r][0] = simfunctions.levenshtein(matrix[2*r][1], matrix[2*r+1][1])  # product type: edit distance
    product_name1 = matrix[2*r][2].split(' ')
    product_name2 = matrix[2*r+1][2].split(' ')
    ...
    FVmatrix[r][3] = simfunctions.levenshtein(matrix[2*r][3], matrix[2*r+1][3]) #product segment: edit distance
    FVmatrix[r][4] = simfunctions.levenshtein(matrix[2*r][4], matrix[2*r+1][4]) #brand: edit distance
    if (matrix[2*r][5] == matrix[2*r+1][5]):
        FVmatrix[r][5] = 1
    else:
        FVmatrix[r][5] = 0
    if (matrix[2*r][6] == matrix[2*r+1][6]):
        FVmatrix[r][6] = 1
    else:
        FVmatrix[r][6] = 0
    FVmatrix[r][7] = simfunctions.levenshtein(matrix[2*r][7], matrix[2*r+1][7]) # category: edit distance
    if(result[r] == 'MATCH\n'):
        FVmatrix[r][8] = 1
    else: 
        FVmatrix[r][8] = 0	
...
x = open('5000Test0_feature_vector.txt','w')
