...
#print(i)
FVmatrix = [[0 for j in range(16)] for j in range(i+1)]    
r = 0
for r in range(i+1):
    ...
    if(matrix[2*r][3] == '' and matrix[2*r+1][3] == ''):
        ...
        FVmatrix[r][8] = simfunctions.soft_tfidf(set(product_seg1), set(product_seg2)) 
    ...
    if(matrix[2*r][4] == '' and matrix[2*r+1][4] == ''):
        ...
        FVmatrix[r][9] = simfunctions.soft_tfidf(set(product_brand1), set(product_brand2)) 		
	#brand: jaro_winkler distance	
    if(matrix[2*r][4] == '' and matrix[2*r+1][4] == ''):
        FVmatrix[r][10] = 999
    else:
        FVmatrix[r][10] = simfunctions.jaro_winkler(matrix[2*r][4], matrix[2*r+1][4]) 
	#brand: jaccard distance	
    if(matrix[2*r][4] == '' and matrix[2*r+1][4] == ''):
        FVmatrix[r][11] = 999
    else:
        FVmatrix[r][11] = simfunctions.jaccard(set(product_brand1), set(product_brand2)) 
		
    # category: jaro_winkler distance
    if(matrix[2*r][5] == '' and matrix[2*r+1][5] == ''):
        FVmatrix[r][12] = 999
    else:
        FVmatrix[r][12] = simfunctions.jaro_winkler(matrix[2*r][5], matrix[2*r+1][5])  		
    ...
    if(matrix[2*r][5] == '' and matrix[2*r+1][5] == ''):
        ...
        FVmatrix[r][13] = simfunctions.soft_tfidf(set(product_category1), set(product_category2))
    # category: jaccard distance
    if(matrix[2*r][5] == '' and matrix[2*r+1][5] == ''):
        FVmatrix[r][14] = 999
    else:
        FVmatrix[r][14] = simfunctions.jaccard(set(product_category1), set(product_category2))
...
x = open('5000Sampling18_string_processed_feature_vector.txt','w')
