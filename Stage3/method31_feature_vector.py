...
x = open('1000Test_matrix.txt','w')
for each in matrix:
    print(each, file = x)
x.close()
#x = open('Y_matrix_class.txt','w')
x = open('1000Test_matrix_class.txt','w')
for each in result:
    print(each, file = x)
x.close()
...
#print(i)
FVmatrix = [[0 for j in range(41)] for j in range(i+1)]    
r = 0
for r in range(i+1):
    ...
    if(matrix[2*r][2] == '' and matrix[2*r+1][2] == ''):
        ...
        FVmatrix[r][9] = simfunctions.levenshtein(matrix[2*r][2], matrix[2*r+1][2])			       
    # product name: needleman_wunsch distance
    if(matrix[2*r][2] == '' and matrix[2*r+1][2] == ''):
        FVmatrix[r][10] = 999
    else:
        FVmatrix[r][10] = simfunctions.needleman_wunsch(matrix[2*r][2], matrix[2*r+1][2])	
    # product name: smith_waterman distance
    if(matrix[2*r][2] == '' and matrix[2*r+1][2] == ''):
        FVmatrix[r][11] = 999
    else:
        FVmatrix[r][11] = simfunctions.smith_waterman(matrix[2*r][2], matrix[2*r+1][2])	
    # product name: affine distance
    if(matrix[2*r][2] == '' and matrix[2*r+1][2] == ''):
        FVmatrix[r][12] = 999
    else:
        FVmatrix[r][12] = simfunctions.affine(matrix[2*r][2], matrix[2*r+1][2])	
    ...
    FVmatrix[r][13] = simfunctions.jaro_winkler(matrix[2*r][2], matrix[2*r+1][2])
    #product name: jaccard score	
    FVmatrix[r][14] = simfunctions.jaccard(set(product_name1), set(product_name2)) 
    #product name: soft_tfidf score	
    FVmatrix[r][15] = simfunctions.soft_tfidf(set(product_name1), set(product_name2))
    #product name: overlap_coefficient score	
    FVmatrix[r][16] = simfunctions.overlap_coefficient(set(product_name1), set(product_name2))
    #product name: cosine score	
    FVmatrix[r][17] = simfunctions.cosine(set(product_name1), set(product_name2))	
	
    # product seg: levenshtein distance
    if(matrix[2*r][3] == '' and matrix[2*r+1][3] == ''):
        FVmatrix[r][18] = 999
    else:
        FVmatrix[r][18] = simfunctions.levenshtein(matrix[2*r][3], matrix[2*r+1][3])			       
    # product seg: needleman_wunsch distance
    if(matrix[2*r][3] == '' and matrix[2*r+1][3] == ''):
        FVmatrix[r][19] = 999
    else:
        FVmatrix[r][19] = simfunctions.needleman_wunsch(matrix[2*r][3], matrix[2*r+1][3])	
    # product seg: smith_waterman distance
    if(matrix[2*r][3] == '' and matrix[2*r+1][3] == ''):
        FVmatrix[r][20] = 999
    else:
        FVmatrix[r][20] = simfunctions.smith_waterman(matrix[2*r][3], matrix[2*r+1][3])	
    # product seg: affine distance
    if(matrix[2*r][3] == '' and matrix[2*r+1][3] == ''):
        FVmatrix[r][21] = 999
    else:
        FVmatrix[r][21] = simfunctions.affine(matrix[2*r][3], matrix[2*r+1][3])	
    ...
    FVmatrix[r][22] = simfunctions.jaro_winkler(matrix[2*r][3], matrix[2*r+1][3]) 
	#product segment: jaccard distance
    FVmatrix[r][23] = simfunctions.jaccard(set(product_seg1), set(product_seg2)) 
	#product segment: soft_tfidf distance
    FVmatrix[r][24] = simfunctions.soft_tfidf(set(product_seg1), set(product_seg2)) 
    #product segment: overlap_coefficient score	
    FVmatrix[r][25] = simfunctions.overlap_coefficient(set(product_seg1), set(product_seg2))
    #product segment: cosine score	
    FVmatrix[r][26] = simfunctions.cosine(set(product_seg1), set(product_seg2)) 
    ...
    if(matrix[2*r][4] == '' and matrix[2*r+1][4] == ''):
        ...
        FVmatrix[r][27] = simfunctions.levenshtein(matrix[2*r][4], matrix[2*r+1][4])			       
    # product brand: needleman_wunsch distance
    if(matrix[2*r][4] == '' and matrix[2*r+1][4] == ''):
        FVmatrix[r][28] = 999
    else:
        FVmatrix[r][28] = simfunctions.needleman_wunsch(matrix[2*r][4], matrix[2*r+1][4])	
    # product brand: smith_waterman distance
    if(matrix[2*r][4] == '' and matrix[2*r+1][4] == ''):
        FVmatrix[r][29] = 999
    else:
        FVmatrix[r][29] = simfunctions.smith_waterman(matrix[2*r][4], matrix[2*r+1][4])	
    # product brand: affine distance
    if(matrix[2*r][4] == '' and matrix[2*r+1][4] == ''):
        FVmatrix[r][30] = 999
    else:
        FVmatrix[r][30] = simfunctions.affine(matrix[2*r][4], matrix[2*r+1][4])	
    ...
    FVmatrix[r][31] = simfunctions.soft_tfidf(set(product_brand1), set(product_brand2)) 		
	#brand: jaro_winkler distance	
    FVmatrix[r][32] = simfunctions.jaro_winkler(matrix[2*r][4], matrix[2*r+1][4]) 
	#brand: jaccard distance	
    FVmatrix[r][33] = simfunctions.jaccard(set(product_brand1), set(product_brand2)) 
	#brand: overlap_coefficient distance	
    FVmatrix[r][34] = simfunctions.overlap_coefficient(set(product_brand1), set(product_brand2))
	#brand: cosine distance	
    FVmatrix[r][35] = simfunctions.cosine(set(product_brand1), set(product_brand2))
    ...
    FVmatrix[r][36] = simfunctions.jaccard(set(product_long1), set(product_long2))
    # long: overlap_coefficient distance
    FVmatrix[r][37] = simfunctions.overlap_coefficient(set(product_long1), set(product_long2))
    # long: cosine distance
    FVmatrix[r][38] = simfunctions.cosine(set(product_long1), set(product_long2))
    # long: soft_tfidf distance
    FVmatrix[r][39] = simfunctions.soft_tfidf(set(product_long1), set(product_long2))
...
x = open('1000Test31_string_processed_feature_vector.txt','w')
