...
#print(i)
FVmatrix = [[0 for j in range(24)] for j in range(i+1)]    
r = 0
for r in range(i+1):
    ...
    if(matrix[2*r][2] == '' and matrix[2*r+1][2] == ''):
        ...
        FVmatrix[r][8] = simfunctions.smith_waterman(matrix[2*r][2], matrix[2*r+1][2])
    # product name: needleman_wunsch distance		
    if(matrix[2*r][2] == '' and matrix[2*r+1][2] == ''):
        FVmatrix[r][9] = 999
    else:
        FVmatrix[r][9] = simfunctions.needleman_wunsch(matrix[2*r][2], matrix[2*r+1][2])
    ...
    if(matrix[2*r][3] == '' and matrix[2*r+1][3] == ''):
	    ...
	    FVmatrix[r][11] = simfunctions.jaro_winkler(matrix[2*r][3], matrix[2*r+1][3]) 
    ...
    if(matrix[2*r][3] == '' and matrix[2*r+1][3] == ''):
        ...
        FVmatrix[r][12] = simfunctions.jaccard(set(product_seg1), set(product_seg2)) 
	#product segment: needleman_wunsch distance
    if(matrix[2*r][3] == '' and matrix[2*r+1][3] == ''):
	    FVmatrix[r][13] = 999
    else:
	    FVmatrix[r][13] = simfunctions.needleman_wunsch(matrix[2*r][3], matrix[2*r+1][3]) 	
	#product segment: soft_tfidf distance
    if(matrix[2*r][3] == '' and matrix[2*r+1][3] == ''):
	    FVmatrix[r][14] = 999
    else:
        FVmatrix[r][14] = simfunctions.soft_tfidf(set(product_seg1), set(product_seg2)) 
		
	#brand: needleman_wunsch distance	
    if(matrix[2*r][4] == '' and matrix[2*r+1][4] == ''):
        FVmatrix[r][15] = 999
    else:		
        FVmatrix[r][15] = simfunctions.needleman_wunsch(matrix[2*r][4], matrix[2*r+1][4])
    ...
    if(matrix[2*r][4] == '' and matrix[2*r+1][4] == ''):
        ...
        FVmatrix[r][16] = simfunctions.soft_tfidf(set(product_brand1), set(product_brand2)) 		
	#brand: jaro_winkler distance	
    if(matrix[2*r][4] == '' and matrix[2*r+1][4] == ''):
        FVmatrix[r][17] = 999
    else:
        FVmatrix[r][17] = simfunctions.jaro_winkler(matrix[2*r][4], matrix[2*r+1][4]) 
	#brand: jaccard distance	
    if(matrix[2*r][4] == '' and matrix[2*r+1][4] == ''):
        FVmatrix[r][18] = 999
    else:
        FVmatrix[r][18] = simfunctions.jaccard(set(product_brand1), set(product_brand2)) 
		
    # category: needleman_wunsch distance
    if(matrix[2*r][5] == '' and matrix[2*r+1][5] == ''):
        FVmatrix[r][19] = 999
    else:
        FVmatrix[r][19] = simfunctions.needleman_wunsch(matrix[2*r][5], matrix[2*r+1][5])
    # category: jaro_winkler distance
    if(matrix[2*r][5] == '' and matrix[2*r+1][5] == ''):
        FVmatrix[r][20] = 999
    else:
        FVmatrix[r][20] = simfunctions.jaro_winkler(matrix[2*r][5], matrix[2*r+1][5])  		
    ...
    if(matrix[2*r][5] == '' and matrix[2*r+1][5] == ''):
        ...
        FVmatrix[r][21] = simfunctions.soft_tfidf(set(product_category1), set(product_category2))
    # category: jaccard distance
    if(matrix[2*r][5] == '' and matrix[2*r+1][5] == ''):
        FVmatrix[r][22] = 999
    else:
        FVmatrix[r][22] = simfunctions.jaccard(set(product_category1), set(product_category2))
...
#x = open('5000Sampling19_string_processed_feature_vector.txt','w')
