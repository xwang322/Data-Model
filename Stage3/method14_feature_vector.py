...
#print(i)
FVmatrix = [[0 for j in range(20)] for j in range(i+1)]    
r = 0
for r in range(i+1):
    ...
    if(matrix[2*r][4] == '' and matrix[2*r+1][4] == ''):
        ...
        FVmatrix[r][11] = simfunctions.needleman_wunsch(matrix[2*r][4], matrix[2*r+1][4])
    ...
    if(matrix[2*r][4] == '' and matrix[2*r+1][4] == ''):
        ...
        FVmatrix[r][12] = simfunctions.soft_tfidf(set(product_brand1), set(product_brand2)) 		
	#brand: jaro_winkler distance	
    if(matrix[2*r][4] == '' and matrix[2*r+1][4] == ''):
        FVmatrix[r][13] = 999
    else:
        FVmatrix[r][13] = simfunctions.jaro_winkler(matrix[2*r][4], matrix[2*r+1][4]) 
	#brand: jaccard distance	
    if(matrix[2*r][4] == '' and matrix[2*r+1][4] == ''):
        FVmatrix[r][14] = 999
    else:
        FVmatrix[r][14] = simfunctions.jaccard(set(product_brand1), set(product_brand2)) 
		
    # category: needleman_wunsch distance
    if(matrix[2*r][5] == '' and matrix[2*r+1][5] == ''):
        FVmatrix[r][15] = 999
    else:
        FVmatrix[r][15] = simfunctions.needleman_wunsch(matrix[2*r][5], matrix[2*r+1][5])
    # category: jaro_winkler distance
    if(matrix[2*r][5] == '' and matrix[2*r+1][5] == ''):
        FVmatrix[r][16] = 999
    else:
        FVmatrix[r][16] = simfunctions.jaro_winkler(matrix[2*r][5], matrix[2*r+1][5])  		
    ...
    if(matrix[2*r][5] == '' and matrix[2*r+1][5] == ''):
        ...
        FVmatrix[r][17] = simfunctions.soft_tfidf(set(product_category1), set(product_category2))
    # category: jaccard distance
    if(matrix[2*r][5] == '' and matrix[2*r+1][5] == ''):
        FVmatrix[r][18] = 999
    else:
        FVmatrix[r][18] = simfunctions.jaccard(set(product_category1), set(product_category2))
...
x = open('5000Test14_string_processed_feature_vector.txt','w')
