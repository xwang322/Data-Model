...
for r in range(i+1):
    ...
    FVmatrix[r][7] = simfunctions.overlap_coefficient(set(product_name1), set(product_name2))
	
	#product segment: jaro_winkler distance
    if(matrix[2*r][3] == '' and matrix[2*r+1][3] == ''):
	    FVmatrix[r][8] = 999
    else:
	    FVmatrix[r][8] = simfunctions.jaro_winkler(matrix[2*r][3], matrix[2*r+1][3]) 
    ...
    if(matrix[2*r][3] == '' and matrix[2*r+1][3] == ''):
        ...
        FVmatrix[r][9] = simfunctions.jaccard(set(product_seg1), set(product_seg2)) 
    ...
    if(matrix[2*r][4] == '' and matrix[2*r+1][4] == ''):
        ...
        FVmatrix[r][11] = simfunctions.soft_tfidf(set(product_brand1), set(product_brand2)) 		
	#brand: jaro_winkler distance	
    if(matrix[2*r][4] == '' and matrix[2*r+1][4] == ''):
        FVmatrix[r][12] = 999
    else:
        FVmatrix[r][12] = simfunctions.jaro_winkler(matrix[2*r][4], matrix[2*r+1][4]) 
	#brand: jaccard distance	
    if(matrix[2*r][4] == '' and matrix[2*r+1][4] == ''):
        FVmatrix[r][13] = 999
    else:
        FVmatrix[r][13] = simfunctions.jaccard(set(product_brand1), set(product_brand2)) 

    # category: jaro_winkler distance
    if(matrix[2*r][5] == '' and matrix[2*r+1][5] == ''):
        FVmatrix[r][14] = 999
    else:
        FVmatrix[r][14] = simfunctions.jaro_winkler(matrix[2*r][5], matrix[2*r+1][5])  		
    ...
    if(matrix[2*r][5] == '' and matrix[2*r+1][5] == ''):
        ...
        FVmatrix[r][15] = simfunctions.soft_tfidf(set(product_category1), set(product_category2))
    # category: jaccard distance
    if(matrix[2*r][5] == '' and matrix[2*r+1][5] == ''):
        FVmatrix[r][16] = 999
    else:
        FVmatrix[r][16] = simfunctions.jaccard(set(product_category1), set(product_category2))
...
x = open('5000Sampling23_string_processed_feature_vector.txt','w')
